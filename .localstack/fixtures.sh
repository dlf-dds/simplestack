...
awslocal kinesis create-stream --stream-name ${sourceStreamName} --shard-count ${NUM_SHARDS} --region ${LOCALSTACK_REGION} --endpoint ${AWS_ENDPOINT}
...
awslocal kinesis create-stream --stream-name ${sinkStreamName} --shard-count ${NUM_SHARDS} --region ${LOCALSTACK_REGION} --endpoint ${AWS_ENDPOINT}
